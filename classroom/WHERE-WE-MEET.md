...
**Stamped:** Fri 2026-09-25, 21:35 MDT (America/Denver), by Grok Bot for the Decider. Status labels: **LIVE** = checked working today. **CODE (0.3.3)** = in the source on branch `build/0.3.3-allos` (RIZALBOT PR #4, unmerged, not yet built or installed). **PLANNED** = not built or not live yet.
...
| **Web (primary name)** | https://rizal.pw/classroom/ | Same page, once rizal.pw serves this repo | PLANNED: rizal.pw now resolves to GitHub Pages, but the domain is bound to `rizalward/rizal.pw` (no `classroom/`) and its HTTPS certificate is not issued yet, so `/classroom/` returns 404 today |

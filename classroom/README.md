...
**Who can approve:** an approval counts only when `from.rid` is the roster Decider `ЯID-0001-3QQS` (and the name matches `roster/`). A `role: "decider"` label alone never counts, so a bot cannot approve itself; validate flags any party claiming the decider role without the Decider's ЯID. `decider_signature` is a reserved, optional field: later the apps will sign approvals with a device key that only unlocks after the Decider's biometric check. It is **not verified yet**.

**Turning an F into a P:** a re-score that supersedes a `passed: false` record with `passed: true` needs `supersede_reason` and `supersede_approved_by_message_id` (a Decider approval in `outbox/`).

Never put a secret in any classroom file, and never name or copy one in free text. The validator rejects files that look like private keys, keypair arrays, or API tokens, and in `pings/`, `door/`, `inbox/` and `outbox/` it also rejects the words *seed phrase*, *private key*, *mnemonic* and *secret key*, runs of 12 or more BIP39 words (wordlist: `tools/bip39-english.txt`, the canonical English list), and base58 strings of 64 to 88 characters. Say "a secret" or "the recovery words" instead, and never copy them. The CLI (`door`, `ping`, `pong`) refuses to write anything without python `jsonschema`, accepts `--at` only as `YYYY-MM-DDTHH:MM:SSZ`, and writes only inside `classroom/`.
